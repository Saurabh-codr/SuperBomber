...
#def notifyen():
 #   try:
  #      if DEBUG_MODE:
   #         url = "https://github.com/Hackertrackersj/SuperBomber/blob/master/.notify"
#        else:
 #           url = "https://github.com/Hackertrackersj/SuperBomber/blob/master/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("SuperBomber was created by Saurabh Coder")
...
def selectnode(mode="sms"):
 ...
 #       notifyen()
        ...
        max_limit = {"sms": 999999, "call": 99999, "mail": 999999}
        cc, target = "", ""
        if mode in ["sms", "call"]:
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("SuperBomber will work only in Python v3")
    sys.exit()

try:
    country_codes = readisdc()["isdcodes"]
except FileNotFoundError:
    mesgdcrt.FailureMessage("Please reinstall the SuperBomber")


__VERSION__ = get_version()
__CONTRIBUTORS__ = ['Saurabh Coder']
...
description = """SuperBomber - Your Friendly Spammer Application

SuperBomber can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

SuperBomber is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by -Saurabh Coder !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start SuperBomber with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start SuperBomber with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start SuperBomber with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current SuperBomber contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current SuperBomber version")
...
if __name__ == "__main__":
    ...
    if args.version:
        ...
        print("Made by : ", " ".join(__CONTRIBUTORS__))
    elif args.mail:
